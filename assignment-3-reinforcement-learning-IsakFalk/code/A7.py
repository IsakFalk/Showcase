...
def main(args):
    ...
    for run in range(runs):
        ...
        with tf.Session() as sess:
            # Create estimator
            estimator = OnlineNeuralNetTargetReplay(sess,
                                                    save_path,
                                                    learning_rate,
            ...
            for episode in range(NUM_EP):
                ...
                states, actions, rewards, next_states, dones = env.get_holders()
                agent.estimator.add_to_buffer(states,
                                              actions,
                                              rewards,
                                              next_states,
                                              dones)
                # Update target network. Counters
                # internally that handles when to copy
                # variables
                agent.estimator.update_target_network()
                # Evaluate agent
