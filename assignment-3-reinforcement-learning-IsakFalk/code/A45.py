...
def main(args):
    ...
    runs = args.runs
    iter_get = args.iter_get
    num_iter = episodes//iter_get
    ...
    iter_episodes = np.arange(0, episodes, iter_get)
    ...
    for run in range(runs):
        ...
        with tf.Session() as sess:
            # Create estimator
            estimator = OnlineNeuralNetVanilla(sess, save_path, learning_rate, hidden_units=args.units)
            # create the agent
            ...
            for episode in range(episodes):
                ...
                if episode % iter_get == 0:
                    ...
                    print('run: {} | episode: {} | Length: {:.2f} | Reward: {:.5f} | Loss: {:.5f}'.format(run,
                                                                                                          episode,
                                                                                                          mean_ep_len,
                                                                                                          mean_reward,
                                                                                                          np.mean(residue ** 2)))
...
if __name__=='__main__':
    ...
    parser.add_argument("-e",
                        "--episodes",
                        dest="episodes",
                        help="Specify the number of episodes to run for.",
                        default=2000,
                        type=int)
    parser.add_argument("-i",
                        "--iter_get",
                        dest="iter_get",
                        help="Specify how often we should save statistics.",
                        default=20,
                        type=int)
